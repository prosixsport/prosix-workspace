<template>
  <AppLayout>
  <div class="place-orders-page artwork-requests-page">
    <aside class="orders-sidebar">
      <div class="sidebar-head">
        ...
        <div>
          ...
          <p>Artwork requests received from Prosix.com</p>
        </div>
      ...
      <div class="search-box">
        ...
        <input v-model="search" type="text" placeholder="Search artwork requests..." />
      </div>
    ...
    <main class="orders-content">
      <div v-if="!selectedOrder" class="empty-detail">
        ...
        <h3>Select a Artwork Request</h3>
  ...
  methods: {
    ...
    async fetchOrders(silent = false) {
      ...
      try {
        ...
        if (!this.selectedOrder && this.orders.length && !silent) {
          this.selectedOrder = this.orders[0]
        }
      } catch (error) {
        console.error('Artwork requests fetch error:', error)
        this.orders = []
      } finally {
        if (!silent) {
          this.loading = false
    ...
    async selectOrder(order) {
      ...
      try {
        ...
        order.is_read = true
        window.dispatchEvent(new CustomEvent('artwork-requests-read-updated'))
      } catch (error) {
        console.error('Artwork request mark-read error:', error)
      }
    },
...
.place-orders-page { width: 100%; min-width: 0; min-height: 100vh; display: grid; grid-template-columns: 360px minmax(0, 1fr); background: #f3f4f6; }
