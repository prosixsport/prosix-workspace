<template>
  <div class="orders-layout" @click="closeAllMenus">

    <!-- LEFT PANEL -->
    <div class="orders-left" :style="{ width: leftWidth + 'px' }">
      ...
      <div v-else-if="filteredOrders.length === 0" class="orders-empty-list">
        No orders found
      </div>
...
<div v-if="isSuperAdmin" class="add-row" @click="addNewOrder">
  ...
  <div class="header-right-icons">
    ...
    >
      ...
      <!-- INFO BAR -->
      <div class="detail-topbar table-border">
        ...
        <div class="detail-info-item" style="position:relative" @click.stop>
          ...
          <span
            class="status-badge"
            :style="{ background: selectedOrder.statusColor }"
  ...
  <div class="custom-status-box">
    <input
      ...
      @keydown.enter.prevent="applyCustomStatus"
    />
    ...
    >
      Add
        ...
        <div v-if="isSuperAdmin" class="detail-info-item invoice-info-item" style="position:relative" @click.stop>
          ...
          <button
            v-if="selectedOrder.invoiceFiles && selectedOrder.invoiceFiles.length"
            class="invoice-view-btn"
            @click="openInvoiceFiles"
          >
            <i class="fa-solid fa-eye me-1"></i>
            View ({{ selectedOrder.invoiceFiles.length }})
          </button>
        ...
        <div class="detail-info-item" style="position:relative" @click.stop>
          ...
          <div v-if="showPaymentMenu" class="payment-dropdown payment-dropdown-wide">
            ...
            <div class="payment-read-row paid-row">
              <span>Paid</span>
              <strong>{{ selectedOrder.payment || '0 % Paid' }}</strong>
            </div>

            <div class="payment-read-row received-row">
              <span>Received</span>
              <strong>${{ selectedOrder.paymentReceived || 0 }}</strong>
            </div>

            <div class="payment-read-row balance-row">
              <span>Balance</span>
              <strong>${{ selectedOrder.paymentBalance || 0 }}</strong>
            </div>
            ...
            <div v-if="isSuperAdmin" class="payment-admin-editor">
              ...
              <div class="payment-field">
                <label class="payment-label">
                  <i class="fa-solid fa-circle-check" style="color:#00c875;margin-right:4px"></i>
                  Received ($)
                </label>
                <input
                  v-model="paymentEdit.received"
                  type="number"
                  min="0"
                  class="payment-input"
              ...
              <div class="payment-field">
                <label class="payment-label">
                  <i class="fa-solid fa-clock" style="color:#fdab3d;margin-right:4px"></i>
                  Balance ($)
                </label>
                <input
                  v-model="paymentEdit.balance"
                  type="number"
                  min="0"
                  class="payment-input"
            ...
            <div v-for="card in selectedOrder.cards" :key="card.title" class="order-card">
              ...
              <div v-if="card.title === 'Notes'" class="card-notes-area">
                ...
                <div class="notes-footer">
                  <span class="notes-count text-dark">
                    {{ card.noteText ? card.noteText.length : 0 }} chars
                  </span>
...
<img
  ...
  :src="file.url"
  class="file-img"
  @click.stop="openPreviewFile(file)"
...
>
  <i class="fa-solid fa-download"></i><span>Download All</span>
        ...
        <!-- CHAT PANEL MOVED TO SEPARATE FILE -->
        <OrderChatPanel
          v-if="showChat"
      ...
      <div class="add-order-modal">
        ...
        <div class="add-order-body">
          <div class="field-group">
            ...
            <input
              ...
              class="field-input"
              placeholder="e.g. DUNBAR FOOTBALL UNIFORMS PO"
              @keyup.enter="confirmAddOrder"
              ref="orderNameInput"
            />
          </div>
          ...
          <div class="field-group">
            ...
            <div class="member-select-actions">
              <button type="button" class="select-all-members-btn" @click="selectAllMembers">
                <i class="fa-solid fa-check-double me-1"></i>Select All
              </button>

              <button type="button" class="select-all-members-btn clear" @click="clearSelectedMembers">
                Clear
              </button>
            </div>

            <Multiselect
              v-model="newOrder.selectedMembers"
        ...
        <div class="add-order-footer">
          ...
          <button class="btn-create" @click="confirmAddOrder" :disabled="savingOrder || !newOrder.name.trim()">
            <span v-if="savingOrder"><i class="fa-solid fa-spinner fa-spin me-1"></i>Saving...</span><span v-else><i class="fa-solid fa-plus me-1"></i>{{ editingOrderId ? 'Update Order' : 'Create Order' }}</span>
          </button>
        </div>
      </div>
    </div>
    ...
    <div v-if="profileModal" class="modal-overlay" @click.self="closeProfile">
      <div class="profile-modal" @click.stop>
        ...
        <button v-if="isOwnProfile(profileUser)" class="btn btn-primary w-100 mt-3" @click="saveProfile">
          Save Profile
        </button>
      </div>
    </div>
      ...
      <div class="view-all-modal">
        <div class="view-all-header">
          ...
          <div class="view-all-head-actions">
            ...
            <button
              v-if="isSuperAdmin && viewAllCard.type !== 'notes'"
              class="upload-small-btn"
              @click="triggerUpload(viewAllCard)"
            >
            ...
            <input
              ...
              type="file"
              multiple
              :ref="'fileInput_modal_' + viewAllCard.title"
              class="hidden-file-input"
              @change="onFileChange($event, viewAllCard)"
            />
            <button class="modal-close" @click="viewAllCard = null">
              <i class="fa-solid fa-xmark"></i>
            </button>
          </div>
        </div>
...
<img
  v-if="file.isImage && !file.imageError"
  :src="file.url"
  class="view-file-img clickable-file-preview"
...
<div v-else class="view-file-doc clickable-file-preview" @click="openPreviewFile(file)">
  <i :class="getFileIcon(file.name)" class="view-file-icon"></i>
              ...
              <div class="view-file-actions">
                <a :href="file.url" :download="file.name" class="vf-btn download-btn">
                  <i class="fa-solid fa-download"></i>
                </a>
                <button v-if="isSuperAdmin" class="vf-btn remove-btn" @click="removeFile(viewAllCard, fi)">
    ...
    <div v-if="previewFile" class="image-preview-overlay" @click.self="previewFile = null">
      <div class="image-preview-modal">
        <button class="image-preview-close" @click="previewFile = null">
          <i class="fa-solid fa-xmark"></i>
        </button>
        ...
        <div v-else class="file-preview-doc">
          ...
          <a :href="previewFile.url" target="_blank" :download="previewFile.name" class="download-all-btn mt-3">
            Download File
          </a>
        </div>
      </div>
...
computed: {
  ...
  userPhoto() {
    return this.currentUser?.profile_photo_url || null
  },

  paymentSummary() {
    const payment = this.selectedOrder?.payment || '0 % Paid'
    const received = this.selectedOrder?.paymentReceived || 0
    const balance = this.selectedOrder?.paymentBalance || 0

    return `${payment} / R $${received} / B $${balance}`
  },
...
methods: {
    ...
    loadCustomStatuses() {
      try {
        ...
        console.error('loadCustomStatuses error:', e)
      }
    },
    ...
    saveCustomStatusOption(status) {
      ...
      if (existsIndex === -1) {
        ...
        this.statusOptions.splice(existsIndex, 1, {
      ...
      const customOnly = this.statusOptions
        ...
        .map(s => ({
          label: s.label,
          color: s.color,
          group: s.group || 'in_production',
          groupLabel: s.groupLabel || 'In Production',
          custom: true
        }))
    ...
    async changeStatusOptionColor(status, color) {
      ...
      if (this.selectedOrder && this.selectedOrder.status === status.label) {
        await this.changeStatus(status)
      }
    },

    selectAllMembers() {
      this.newOrder.selectedMembers = [...this.availableMembers]
    },

    clearSelectedMembers() {
      this.newOrder.selectedMembers = []
    },
    ...
    async onInvoiceFileChange(event) {
      ...
      try {
        await axios.post(`/api/orders/${this.selectedOrder.id}/files`, formData, {
          headers: {
            ...this.headers(),
        ...
        alert(e.response?.data?.message || 'Invoice upload nahi hui')
      }
    },
    ...
    async saveTracking() {
      ...
      try {
        ...
        alert(e.response?.data?.message || 'Tracking save nahi hua')
      }
    },
    ...
    async downloadAllFiles(card) {
      ...
      try {
        ...
        console.error('downloadAllFiles error:', e)
        alert('Download all ZIP nahi ban saka')
      }
    },

    isOwnProfile(user) {
      return Number(user?.id || 0) === Number(this.currentUser?.id || 0)
    },
    ...
    async openProfile(user) {
      ...
      try {
        ...
        this.profileForm = {
          ...
          profile_photo: null,
          preview: res.data?.profile_photo_url || ''
        }

        this.profileModal = true
      } catch (e) {
        console.error('openProfile error:', e)
        alert(e.response?.data?.message || 'Profile load nahi hui')
      }
    },
    closeProfile() {
...
async toggleChat() {
  ...
  if (this.showChat && this.selectedOrder) {
    ...
    // close hone par dobara unread check
  ...
  try {
    ...
    const res = await axios.get(
      `/api/orders/${this.selectedOrder.id}/messages/unread-count`,
      {
        headers: this.headers()
      }
    )
    ...
    async markChatRead() {
      ...
      try {
        await axios.post(`/api/orders/${this.selectedOrder.id}/messages/mark-read`, {}, {
          headers: this.headers()
        })
    ...
    formatOrder(order) {
      ...
      return {
        ...
        owners: members.map(m => ({
          ...
          name: m.name,
          email: m.email,
          initial: this.initial(m.name),
          color: this.memberColor(m.id),
 ...
 async fetchMessages(orderId) {
  ...
  this.$nextTick(() => {
    this.$refs.orderNameInput?.focus()
  })
},
    ...
    toggleOrderMenu(id) {
      this.openOrderMenuId = this.openOrderMenuId === id ? null : id
    },
    ...
    openEditOrder(order) {
      ...
      this.newOrder = {
        name: order.name || '',
        po: order.po === 'N/A' ? '' : (order.po || ''),
        selectedMembers: (order.members || []).map(member => {
          return this.availableMembers.find(m => Number(m.id) === Number(member.id)) || member
        }),
        shipDate: order.shipDateRaw || '',
        status: order.status || 'Pending',
        payment: order.payment || '0 % Paid',
        trk: order.trk === 'N/A' ? '' : (order.trk || 'N/A')
      }
    ...
    getOrderNote(order) {
      return order.cards?.find(c => c.title === 'Notes')?.noteText || ''
    },
    ...
    async duplicateOrder(order) {
      ...
      try {
        const res = await axios.post('/api/orders', {
          ...
          po: order.po === 'N/A' ? '' : order.po,
          member_ids: (order.members || []).map(m => m.id),
          ship_date: order.shipDateRaw || null,
          status: order.status,
          status_color: status?.color || order.statusColor || '#fdab3d',
          trk: order.trk || 'N/A',
          payment: order.payment || '0 % Paid',
          notes: this.getOrderNote(order)
        }, {
          headers: this.headers()
        })
        ...
        console.error('duplicateOrder error:', e)
        alert(e.response?.data?.message || 'Order duplicate nahi hua')
      }
    },
    ...
    async deleteOrder(order) {
      ...
      try {
        ...
        alert(e.response?.data?.message || 'Order delete nahi hua')
      }
    },
    ...
    async confirmAddOrder() {
      ...
      const payload = {
        ...
        ship_date: this.newOrder.shipDate || null,
        status: this.newOrder.status,
        status_color: status?.color || '#fdab3d',
        trk: this.newOrder.trk || 'N/A',
      ...
      try {
        ...
        if (this.editingOrderId) {
          res = await axios.put(`/api/orders/${this.editingOrderId}`, payload, {
            headers: this.headers()
          })
        } else {
        ...
        // ✅ Edit order ke baad fresh orders load, taake members updated show hon
        await this.fetchOrders()
        ...
        console.error('confirmAddOrder error:', e)
        alert(e.response?.data?.message || 'Order save nahi hua')
      } finally {
        this.savingOrder = false
      }
    },
    ...
    async savePayment() {
      ...
      try {
        await axios.put(`/api/orders/${this.selectedOrder.id}`, {
          payment,
          payment_received: this.paymentEdit.received || 0,
          payment_balance: this.paymentEdit.balance || 0
        }, {
          headers: this.headers()
        })
  ...
  try {
    ...
    resolveFileUrl(file) {
      let url = file.url || file.file_url || file.full_url || file.path || ''
...
normalizeOrderFile(file) {
  ...
  return {
    ...
    sender: file.user?.name || 'Shared file',
    senderId: file.user?.id || file.user_id || null,
    senderInitial: this.initial(file.user?.name || 'F'),
    senderColor: this.memberColor(file.user?.id || file.user_id || 0),
    senderPhoto: file.user?.profile_photo_url || null,
    saved: true,
    imageError: false
  }
},
...
this.selectedOrder.cards.forEach(card => {
  ...
  if (card.type === 'order_files') {
    card.files = normalizedFiles.filter(file =>
      file.cardType === 'order_files' || file.cardType === 'chat_files'
    )
  } else {
    card.files = normalizedFiles.filter(file => file.cardType === card.type)
  }
})

    // ✅ refresh ke baad chat files dobara show
    this.syncChatFileMessages(
      normalizedFiles.filter(file => file.cardType === 'chat_files')
    )
  } catch (e) {
    console.error('fetchOrderFiles error:', e)
  }
},
    ...
    mergeFiles(oldFiles = [], newFiles = []) {
      ...
      oldFiles.forEach(file => {
        const key = file.id || file.url || `${file.name}-${file.size}`
        map.set(String(key), file)
      })
...
syncChatFileMessages(chatFiles) {
  ...
  const fileMessages = chatFiles.map(file => {
    ...
    return {
      localKey: `file-${file.id}`,
      fileMessageId: file.id,
      senderId: file.senderId || file.user_id || null,
      sender: file.sender || 'Shared file',
  ...
  this.chatMessages = [...normalMessages, ...fileMessages]
    ...
    async uploadFilesToOrder(files, cardType) {
      ...
      // ✅ Real-time preview: old files keep, new temp previews add
      ...
      try {
        const res = await axios.post(`/api/orders/${this.selectedOrder.id}/files`, formData, {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
            Accept: 'application/json'
          }
    ...
    async onFileChange(event, card) {
      ...
      try {
        ...
        alert(e.response?.data?.message || 'File upload nahi hui')
      }
    },
    ...
    async removeFile(card, index) {
      ...
      try {
        await axios.delete(`/api/order-files/${file.id}`, {
          headers: this.headers()
        })
    ...
    openInvoiceFiles() {
      ...
      this.viewAllCard = {
        title: 'Invoices',
        type: 'invoice_files',
        files: this.selectedOrder.invoiceFiles
      }
    },
...
.list-head {
  display:grid;
  grid-template-columns:32px 1fr 80px 38px;
  padding:6px 0;
  font-size:11px;
  font-weight:700;
  color:#5c6180;
...
.file-thumb:hover .file-remove-btn {
  opacity:1;
}
...
.file-thumb:has(.uploading-label) {
  opacity:0.85;
}
...
/* PAYMENT READ ONLY / ADMIN EDIT */
.payment-dropdown-wide{
  width:280px;
}
...
.payment-read-row strong{
  color:#172b4d;
  font-size:13px;
}

.payment-admin-editor{
  border-top:1px solid #e5e7eb;
  margin-top:6px;
  padding-top:4px;
}
...
.payment-read-row.paid-row{
  background:#eef2ff;
}

.payment-read-row.received-row{
  background:#e8fff3;
}

.payment-read-row.balance-row{
  background:#fff4e5;
}
...
.custom-status-color::-webkit-color-swatch-wrapper{
  padding:0;
}

.custom-status-color::-webkit-color-swatch{
  border:none;
  border-radius:50%;
}

/* UPDATED PAYMENT TOP LABELS */
...
.payment-chip-received{
  background:#e8fff3;
  color:#00a86b;
}

.payment-chip-balance{
  background:#fff4e5;
  color:#d97706;
}
