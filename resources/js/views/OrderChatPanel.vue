<template>
  ...
  >
    <div class="chat-resize-bar" @mousedown.stop="startChatResize"></div>
    ...
    <div class="team-members-list">
      ...
      <div v-if="showMembersList" class="team-members-scroll">
        ...
        <div v-if="teamMembers.length === 0" class="member-drop-empty">
    ...
    <div class="chat-messages" ref="chatMessages">
      ...
      >
        <div class="chat-msg-meta">
          ...
          >
            <img
              v-if="messagePhoto(msg)"
              :src="messagePhoto(msg)"
              class="member-av-img"
            />
        ...
        <div class="chat-bubble-wrap">
          <div class="chat-bubble-user">
            <div v-if="isDeletedEveryone(msg)" class="deleted-message">
              {{ deletedText(msg) }}
            </div>
          ...
          <button
            ...
            @click.stop="toggleMessageMenu(msg, i)"
          >
            <i class="fa-solid fa-chevron-down"></i>
          </button>

          <div
            v-if="openMessageMenuKey === messageKey(msg, i)"
          ...
          >
            ...
            <div
              class="msg-action-item"
              @click="openMessageInfo(msg)"
            >
              <i class="fa-solid fa-circle-info"></i> Info
            </div>

            <div
              class="msg-action-item danger"
              @click="openDeleteOptions(msg)"
            >
    ...
    <div v-if="deleteTarget" class="delete-modal-overlay" @click.self="deleteTarget = null">
      <div class="delete-modal">
        ...
        <button class="delete-modal-option" @click="deleteForMe">
          Delete for me
        </button>

        <button
          v-if="canDeleteForEveryone(deleteTarget)"
          class="delete-modal-option danger"
          @click="deleteForEveryone"
        >
          Delete for everyone
        </button>

        <button class="delete-modal-option cancel" @click="deleteTarget = null">
          Cancel
        </button>
      </div>
    </div>
      ...
      <div class="message-info-modal">
        ...
        <div class="info-section">
          ...
          <div v-if="seenList(infoTarget).filter(u => Number(u.id) !== Number(currentUser?.id || 0)).length">
            <div
              v-for="u in seenList(infoTarget).filter(u => Number(u.id) !== Number(currentUser?.id || 0))"
              :key="u.id"
              class="seen-info-row"
            >
              <span>{{ u.name }}</span>
          ...
          <div v-else class="seen-info-empty">
            Not seen yet
          </div>
        </div>

        <button class="delete-modal-option cancel" @click="infoTarget = null">
          Close
        </button>
      </div>
    </div>
    ...
    <div v-if="profileModal" class="profile-modal-overlay" @click.self="closeProfile">
      <div class="profile-modal" @click.stop>
        ...
        <button v-if="isOwnProfile" class="btn btn-primary w-100 mt-3" @click="saveProfile">
          Save Profile
        </button>
      </div>
    </div>
...
export default {
  ...
  computed: {
    ...
    isOwnProfile() {
      return Number(this.profileUser?.id || 0) === Number(this.currentUser?.id || 0)
    },
    ...
    filteredAvailableMembers() {
      return this.availableMembers.filter(m =>
        !this.teamMembers.some(tm => tm.id === m.id)
      )
    },
    ...
    tagFilteredMembers() {
      ...
      return this.teamMembers.filter(m =>
        m.name.toLowerCase().includes(this.tagQuery.toLowerCase())
      )
    }
  },
...
watch: {
  ...
  methods: {
    async openProfile(member) {
      ...
      try {
        const res = await axios.get(`/api/users/${member.id}/profile`, {
    ...
    messagePhoto(msg) {
      ...
      if (senderId && Number(this.currentUser?.id || 0) === senderId) {
    ...
    messageKey(msg, index) {
      return msg.localKey || msg.id || msg.fileMessageId || `msg-${index}`
    },

    isMine(msg) {
      if (!this.currentUser?.id) return false
      return Number(msg.senderId || 0) === Number(this.currentUser.id)
    },

    isDeletedEveryone(msg) {
      return Boolean(msg.deletedEveryoneAt || msg.deleted_everyone_at)
    },

    deletedText(msg) {
      return this.isMine(msg) ? 'You deleted this message' : 'This message was deleted'
    },

    seenList(msg) {
      return msg.seenBy || (msg.reads || []).map(r => ({
    ...
    canShowMessageActions(msg) {
      return Boolean(msg.id) && !msg.fileMessageId && !this.isDeletedEveryone(msg)
    },

    canEditMessage(msg) {
      return this.isMine(msg) && Boolean(msg.id) && Boolean(msg.text) && !this.isDeletedEveryone(msg)
    },

    canDeleteForEveryone(msg) {
      return this.isMine(msg) && Boolean(msg.id) && !this.isDeletedEveryone(msg)
    },

    isEditing(msg) {
      return this.editingMessageId && Number(this.editingMessageId) === Number(msg.id)
    },

    displaySender(msg) {
      if (msg.senderId && this.currentUser?.id && Number(msg.senderId) === Number(this.currentUser.id)) {
        return 'You'
      }

      if (!msg.senderId && msg.sender === this.currentUser?.name) {
        return 'You'
      }
    ...
    displayInitial(msg) {
      const name = this.displaySender(msg)
    ...
    async saveEditMessage() {
      ...
      try {
        const res = await axios.put(
          `/api/orders/${this.selectedOrder.id}/messages/${this.editingMessageId}`,
          { message: text },
          { headers: this.headers() }
        )
        ...
        console.error('saveEditMessage error:', e)
        alert(e.response?.data?.message || 'Message edit nahi hua')
      }
    },

    openDeleteOptions(msg) {
    ...
    async deleteForMe() {
      ...
      try {
        if (target.id && !target.fileMessageId) {
          await axios.delete(
            `/api/orders/${this.selectedOrder.id}/messages/${target.id}/for-me`,
            { headers: this.headers() }
          )
        }

        const nextMessages = this.chatMessages.filter(msg =>
          this.messageKey(msg, 0) !== this.messageKey(target, 0)
        )
        ...
        alert(e.response?.data?.message || 'Delete for me nahi hua')
      }
    },
    ...
    async deleteForEveryone() {
      ...
      try {
        const res = await axios.delete(
          `/api/orders/${this.selectedOrder.id}/messages/${target.id}/everyone`,
          { headers: this.headers() }
        )
        ...
        alert(e.response?.data?.message || 'Delete for everyone nahi hua')
      }
    },
    ...
    async sendMessage() {
      ...
      try {
        const res = await axios.post(`/api/orders/${this.selectedOrder.id}/messages`, {
        ...
        alert(e.response?.data?.message || 'Message send nahi hua')
      }
    },
    ...
    async uploadChatFiles(event) {
      ...
      const tempMessages = tempFiles.map((file, index) => ({
        ...
        senderId: me.id || null,
        sender: me.name || 'You',
        senderInitial: this.initial(me.name || 'You'),
        senderColor: this.memberColor(me.id || 0),
        senderPhoto: me.profile_photo_url || null,
        time: now,
        sortAt: tempSortAt + index,
        text: '',
        files: [file]
      }))
      ...
      try {
        const res = await axios.post(`/api/orders/${this.selectedOrder.id}/chat-files`, formData, {
          headers: {
            ...this.headers(),
            'Content-Type': 'multipart/form-data'
          }
        ...
        const savedFiles = savedFilesRaw.map(file => ({
          ...
          cardType: 'chat_files',
          senderId: file.user?.id || me.id || null,
          senderPhoto: file.user?.profile_photo_url || me.profile_photo_url || null
        }))
        ...
        const savedMessages = savedFiles.map((file, index) => {
          ...
          return {
            ...
            fileMessageId: file.id,
            senderId: file.senderId || me.id || null,
            sender: file.sender || me.name || 'You',
    ...
    syncChatFileMessages(chatFiles) {
      const existingIds = new Set(
        this.chatMessages
          .filter(msg => msg.fileMessageId)
          .map(msg => msg.fileMessageId)
      )

      const fileMessages = chatFiles
        ...
        .map(file => {
          ...
          return {
            localKey: `file-${file.id}`,
            fileMessageId: file.id,
            senderId: file.senderId || null,
            sender: file.sender || 'Shared file',
            senderInitial: file.senderInitial || 'F',
            senderColor: file.senderColor || '#6161ff',
      ...
      if (fileMessages.length) {
        this.$emit('update-chat-messages', [...this.chatMessages, ...fileMessages])
        this.scrollChatBottom()
      }
    },
    ...
    async addMemberToOrder() {
      ...
      try {
        await axios.post(`/api/orders/${this.selectedOrder.id}/members`, {
        ...
        console.error('addMemberToOrder error:', e)
        alert(e.response?.data?.message || 'Member add nahi hua')
      }
    },
    ...
    async removeMember(userId) {
      ...
      try {
        await axios.delete(`/api/orders/${this.selectedOrder.id}/members/${userId}`, {
          headers: this.headers()
        })
        ...
        alert(e.response?.data?.message || 'Member remove nahi hua')
      }
    },
...
.chat-msg-group.mine .chat-bubble-user {
  margin-left:0;
  margin-right:34px;
...
.chat-msg-group.mine .chat-bubble-wrap {
  flex-direction: row-reverse;
}
...
.chat-msg-group:hover .chat-action-btn {
  opacity: 1;
}
...
.chat-msg-group:not(.mine) .msg-action-menu {
  left: 0;
  right: auto;
}
...
.delete-modal-option:hover {
  background: #f5f6fb;
}

.delete-modal-option.danger {
  color: #e2445c;
}

.delete-modal-option.cancel {
  color: #6b7280;
}

/* PROFILE + COLLAPSIBLE MEMBERS */
.member-toggle{
  display:flex;
  align-items:center;
  justify-content:space-between;
...
.member-av:hover,
.member-info:hover{
  box-shadow:0 0 0 2px rgba(97,97,255,.18);
}
